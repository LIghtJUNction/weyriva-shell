...
`id` is any scalar JSON value. `params` is forwarded as one JSON value to plugins. Built-ins are `weyriva.ping`, `weyriva.info`, `weyriva.methods`, `weyriva.plugin.list`, `weyriva.plugin.reload`, `weyriva.launcher.open`, `weyriva.notifications.dismiss_all`, `weyriva.notifications.dnd`, `weyriva.panel.toggle`, `weyriva.panel.reload`, `weyriva.niri.outputs`, and `weyriva.niri.windows`. Action methods accept fixed executable arguments: callers cannot supply commands.

`weyriva.plugin.reload` makes the running daemon rescan plugin manifests and answers with the refreshed plugin summary; in-flight requests keep the registry they started with. `weyriva.niri.outputs` and `weyriva.niri.windows` return niri's own JSON for connected outputs and open windows, so panels and scripts can read compositor state through one socket.
